...
class Role_model extends CI_Model
{
     ...
     * @return boolean $result : TRUE / FALSE
     */
    function deleteRole($roleId, $userInfo)
    {
        $this->db->where('roleId', $roleId);
    ...
    function getRow(Array $condition, Array $select_columns = array('*'), $order_by = 'DESC')
    {
        $this->db->select($select_columns);
        $this->db->from('tbl_roles AS rl');
        $this->db->where($condition);
        $this->db->where('rl.isDeleted', 0);
        $this->db->order_by('rl.roleId', $order_by);
        $query = $this->db->get();
